class ControlPanel
  ...
  def station_menu
    loop do
      ...
      when '3' then show_trains_at_station
      when '0' then break
      else puts 'Wrong input'
      end
    end
  ...
  def train_menu
    loop do
      ...
      when '3' then edit_car
      when '4' then show_cars_in_train
      when '5' then delete_car_from_train
      when '6' then show_trains
      when '7' then show_trains_at_station
      when '8' then add_route_to_train
      when '9' then move_train_forwards
      when '10' then move_train_backwards
      when '0' then break
      else puts 'Wrong input'
      end
    end
  ...
  def show_train_menu
    ...
    puts 'Enter \'5\' to remove car from existing train'
    puts 'Enter \'6\' to get a list of trains'
    puts 'Enter \'7\' to get a list of trains at station'
    puts 'Enter \'8\' to add route to existing train'
    puts 'Enter \'9\' to move train forwards'
    puts 'Enter \'10\' to move train backwards'
    puts 'Enter \'0\' to return to previous menu'
  end
  ...
  def add_car_to_train
    ...
    return unless train
    if train.type == :cargo
      print "Enter space volume: "
      car = CargoCar.new(gets.chomp.to_i)
    else
      print "Enter number of seats: "
      car = PassengerCar.new(gets.chomp.to_i)
    end
    train.add_car(car)
    puts "\nCar was added!"
  rescue ArgumentError => e
    puts "\n#{e.message}"
    puts 'Car was not added! Try again!'
    add_car_to_train
  end
  ...
  def edit_car
    ...
    return unless car
    if car.type == :cargo
      print 'Enter how much space to take: '
      if car.take_space(gets.chomp.to_i)
  ...
  def show_trains_at_station
    ...
    if station.trains.any?
      ...
      station.each_train_with_index { |train, index| puts "#{index}. #{train.info}" }
    else
  ...
  def choose_object(name, array, callback)
    ...
    when 'cars' then show_cars(array)
    end
    print 'Choose index: '
